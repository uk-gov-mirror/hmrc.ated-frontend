...
class ViewReliefReturnControllerSpec extends PlaySpec with GuiceOneServerPerSuite with MockitoSugar with BeforeAndAfterEach with MockAuthUtil {
  ...
  class Setup {
    ...
    def getWithAuthorisedUserSuccess(test: Future[Result] => Any, isEditable: Boolean = true): Unit = {
      ...
      when(mockReliefsService.viewReliefReturn(ArgumentMatchers.eq(periodKey), ArgumentMatchers.eq(formBundleNo))(using ArgumentMatchers.any(), ArgumentMatchers.any()))
    ...
    def submitWithAuthorisedUser(test: Future[Result] => Any): Unit = {
      ...
      when(mockBackLinkCacheService.saveBackLink(ArgumentMatchers.any(), ArgumentMatchers.any())(using ArgumentMatchers.any())).thenReturn(Future.successful(Some("http://backLink")))
