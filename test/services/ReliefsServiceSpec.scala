  ...
  "ReliefsService" must {
    "save the draft reliefs" must {
      "Create a default Tax Avoidance if we don't already have one" in new Setup {
        ...
        when(mockAtedConnector.retrievePeriodDraftReliefs(ArgumentMatchers.eq("ATED-123"), ArgumentMatchers.eq(periodKey))(using any(), any()))
        ...
        when(mockAtedConnector.saveDraftReliefs(any(), captor.capture())(using any(), any())).thenReturn(Future.successful(HttpResponse(OK, responseJson.toString)))
      ...
      "overwrite the current Tax Avoidance, if the tax avoidance option isn't set" in new Setup {
        ...
        val responseJson: JsValue = Json.toJson(reliefsTaxAvoidance)
        when(mockAtedConnector.retrievePeriodDraftReliefs(any(), any())(using any(), any())).thenReturn(Future.successful(HttpResponse(OK, responseJson.toString)))
      ...
      "throw Internal server exception , for any other status code" in new Setup {
        ...
        val captor: ArgumentCaptor[ReliefsTaxAvoidance] = ArgumentCaptor.forClass(classOf[ReliefsTaxAvoidance])
        when(mockAtedConnector.saveDraftReliefs(any(), captor.capture())(using any(), any())).thenReturn(Future.successful(HttpResponse(BAD_GATEWAY, respJson.toString)))
    ...
    "Save the tax avoidance" must {
      "Create a default Reliefs if we don't already have one and wipe the tax avoidance" in new Setup {
        ...
        when(mockAtedConnector.retrievePeriodDraftReliefs(any(), any())(using any(), any())).thenReturn(Future.successful(HttpResponse(OK, respJson.toString)))
    ...
    "Retrieve the draft reliefs" must {
      ...
      "take no action, for any Not Found exception" in new Setup {
        ...
        when(mockAtedConnector.retrievePeriodDraftReliefs(any(), any())(using any(), any())).thenReturn(Future.successful(HttpResponse(NOT_FOUND, "")))
    ...
    "Submit the draft reliefs" must {
      "successfully submit reliefs to ETMP via DES" in new Setup {
        ...
        when(mockAtedConnector.submitDraftReliefs(any(), any())(using any(), any())).thenReturn(Future.successful(HttpResponse(OK, successResponse.toString)))
        ...
        when(mockDataCacheService.saveFormData[SubmitReturnsResponse](ArgumentMatchers.eq(SubmitReturnsResponseFormId),
          ArgumentMatchers.eq(successResponse.as[SubmitReturnsResponse]))(using ArgumentMatchers.any(), ArgumentMatchers.any()))
        ...
        verify(mockDataCacheService, times(1)).clearCache()(using ArgumentMatchers.any())
        verify(mockDataCacheService, times(1)).saveFormData(any(),any())(using any(),any())
      ...
      "Return 404 when there are no reliefs to submit" in new Setup{
        ...
        when(mockAtedConnector.submitDraftReliefs(any(), any())(using any(), any())).thenReturn(Future.successful(HttpResponse(NOT_FOUND, notFoundResponse.toString)))
        ...
        when(mockDataCacheService.saveFormData[AlreadySubmittedReturnsResponse](ArgumentMatchers.eq(AlreadySubmittedReturnsResponseFormId),
          ArgumentMatchers.eq(notFoundResponse.as[AlreadySubmittedReturnsResponse]))(using ArgumentMatchers.any(), ArgumentMatchers.any()))
        ...
        verify(mockDataCacheService, times(2)).clearCache()(using ArgumentMatchers.any())
        verify(mockDataCacheService, times(2)).saveFormData(any(),any())(using any(),any())
      ...
      "throw Internal Server Error for any other status code" in new Setup {
        ...
        when(mockAtedConnector.submitDraftReliefs(any(), any())(using any(), any())).thenReturn(Future.successful(HttpResponse(BAD_REQUEST, badRequestResponse.toString)))
    ...
    "View relief return" must {
      ...
      "if summary data is found in Cache, return Some EtmpReliefReturnsSummary" in new Setup {
        when(mockDataCacheService.fetchAndGetData[SummaryReturnsModel](ArgumentMatchers.eq(RetrieveReturnsResponseId))(using ArgumentMatchers.any(), ArgumentMatchers.any()))
    ...
    "delete relief draft returns from period key" must {
      ...
      "delete the draft return from mongodb" in new Setup {
        ...
        when(mockAtedConnector.deleteDraftReliefsByYear(any())(using any(), any())) thenReturn Future.successful(HttpResponse(OK, ""))
